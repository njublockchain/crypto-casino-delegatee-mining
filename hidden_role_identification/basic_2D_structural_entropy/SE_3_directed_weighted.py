...
# vanilla_2D_SE_mini：适用于全局优化，没有社区数量的限制。
# hier_2D_SE_mini：适用于需要控制社区数量的情况，同时尽量优化结构熵。
...
def test_SE():
    '''
    Test SE with a directed weighted graph.
    '''
    A = np.array([
        [0, 0, 1, 1, 1],  # 节点0指向节点2、3、4，权重为1
        [0, 0, 0, 0, 2],  # 节点1指向节点4，权重为2
        [0, 1, 0, 1, 0],  # 节点2指向节点1、3，权重为1
        [0, 3, 0, 0, 0],  # 节点3指向节点1，权重为3
        [4, 1, 1, 0, 0]   # 节点4指向节点0、1、2，权重分别为4、1、1
    ])
